...
public class SwipeLeftLayout extends FrameLayout {
    private static final String TAG = SwipeLeftLayout.class.getSimpleName();
    ...
    public SwipeLeftLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SwipeLeftLayout(Context context, AttributeSet attrs, int defStyle) {
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (!animation && once && deltaX < -mTouchSlop && Math.abs(deltaY) < Math.abs(deltaX)) {
                    once = false;
                    isSilding = true;
                    return true;
                }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            case MotionEvent.ACTION_MOVE:
                ...
                tempX = moveX;
                if (isSilding && !animation) {
                    mContentView.scrollBy(deltaX, 0);
                }
                break;
            case MotionEvent.ACTION_UP:
                if (!animation && isSilding) {
                    isSilding = false;
    ...
    private void scrollLeft() {
        ...
        mScroller.startScroll(mContentView.getScrollX(), 0, delta, 0, Math.abs(delta) / 2);
        postInvalidate();
    }
    ...
    public void computeScroll() {
        ...
        animation = mScroller.computeScrollOffset();
