    ...
    public AbsListView.OnScrollListener makeScrollListener() {
        return new AbsListView.OnScrollListener() {
            ...
            public void onScrollStateChanged(AbsListView absListView, int scrollState) {
                ...
                if (scrollState == 0) {
                    if (list.size() == 0) {
                        if (SwipeDismissListViewTouchListener.isFinish == true && position == 0) {
                            SwipeDismissListViewTouchListener.isFinish = false;
                            mCallbacks.setFlag(0);
                        }
                    }
            ...
            public void onScroll(AbsListView absListView, int i, int i1, int i2) {
                position = absListView.getFirstVisiblePosition();
                if (list.size() == 2 && list.get(1) != position) {
                    list.remove(0);
                    list.add(position);
                }
                if (list.size() == 1 && list.get(0) != position) {
                    list.add(position);
                }
                if (list.size() == 0) {
                    list.add(position);
    ...
    public boolean onTouch(View view, MotionEvent motionEvent) {
        ...
        switch (motionEvent.getActionMasked()) {
            case MotionEvent.ACTION_DOWN: {
                ...
                Log.d("NotificationList", "downY = " + downY + ", " + isFinish);
                if (mPaused) {
                    return false;
                }
            ...
            case MotionEvent.ACTION_UP: {
                if (mListView.getFirstVisiblePosition() == 0) {
                    if ((moveY - downY) > 40) {
                        isFinish = true;
                        Log.d("NotificationList", "(moveY - downY) = " + (moveY - downY) + ", " + isFinish);
                    } else {
                        isFinish = false;
                        Log.d("NotificationList", "(moveY - downY) = " + (moveY - downY) + ", " + isFinish);
                    }
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                Log.d("NotificationList", "moveY = " + moveY);
