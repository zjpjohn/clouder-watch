 ...
 */
public class RFCommServiceHandler extends Thread {

    private static final String TAG = "RFCommServiceHandler";
    ...
    private IRFCommHandlerCallback callback;

    public RFCommServiceHandler(BluetoothSocket bluetoothSocket, IRFCommHandlerCallback rfCommHandlerCallback) {

        this.bluetoothSocket = bluetoothSocket;

        this.callback = rfCommHandlerCallback;
        try {
    ...
    public void run() {
        if (inputStream == null || outputStream == null) {
            callback.onRFCommDisconnected(stopped);
            return;
        }
        while (!stopped) {
            ...
            try {
                ...
                if (!stopped) {
                    callback.onRFCommDisconnected(false);
                }
                close();
                break;
            }
    ...
    interface IRFCommHandlerCallback {

        void onRFCommDisconnected(boolean normalStop);
