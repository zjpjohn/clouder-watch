...
public class TransportParser {
	...
	public static byte[] dataPack(TransportData data) throws IOException {
		...
		LogTool.e(TAG, "TransportData = " + data.toString());

		byte[] content = new byte[readLength];
		LogTool.e(TAG, "readLength = " + readLength);
	...
	public static TransportData dataUnpack(byte[] dataPack) {
		...
		try {
			...
			if (checkDataPack(dataPack)) {
				...
				String filepath = basePath + "//" + dataObj.getUuid().toString();
				...
				LogTool.e(TAG, "dataUnpack datat is invalid.");
			}
		} catch (Exception e) {
	...
	public static boolean checkDataPack(byte[] dataPack) {
		...
		if (dataPack.length < 2 + 4 + 1 + 2 + 2) {
			LogTool.e(TAG, "[checkDataPack] 长度小于规定");
			return false;
		}
		// check pack length
		byte l0 = dataPack[2];
		...
		if (dataPack.length != length) {
			...
			LogTool.e(TAG, "[checkDataPack] 长度不一致");
			return false;
		}
		...
		if (newCrc[0] != crc1 || newCrc[1] != crc0) {
			// error crc
			LogTool.e(TAG, "[checkDataPack] CRC不一致");
			return false;
		}
		return true;
	}
	...
	public static void main(String[] args) throws IOException {
		...
		data.setCount(100100056);
		data.setIndex(10010);
		data.setContentLength(10);
		data.setProtocolType((byte) 1);
		String uuid = UUID.randomUUID().toString();
		data.setUuid(uuid);
		byte[] content = new byte[] { 1, 2, -1, 3, 4, 7, 9, 1, 9, 10 };
		System.out.println("发送前：" + data.toString());
		byte[] tmp = dataPack(data);
		TransportData data2 = dataUnpack(tmp);
		System.out.println("发送后：" + data2.toString());
	}
