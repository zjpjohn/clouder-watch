...
public class GetConnectedNodesResponse implements SafeParcelable {
	...
	public static final Creator<GetConnectedNodesResponse> CREATOR = new Creator<GetConnectedNodesResponse>() {
	...
	public GetConnectedNodesResponse(Parcel parcel) {
		...
		for (int i = 0; i < arrayOfParcelable.length; i++) {
			NodeHolder nodeHolder = (NodeHolder) arrayOfParcelable[i];
			this.nodes.add(nodeHolder);
		}
	}
