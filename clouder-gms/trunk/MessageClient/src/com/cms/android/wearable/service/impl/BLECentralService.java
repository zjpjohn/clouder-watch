...
public class BLECentralService extends Service {
	...
	private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
			...
			if (status == BluetoothGatt.GATT_SUCCESS) {
				...
				try {
					...
					if (null != service) {
						...
						writeCharacteristic(BleUuid.SERVICE_DEVICE_INFORMATION, BleUuid.CHAR_NODE_LOCAL, nodeId);
					}
		...
		public void onCharacteristicWrite(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
			...
			if (characteristic.getUuid().toString().indexOf(BleUuid.CHAR_NODE_LOCAL) > -1) {
				writeCharacteristic(BleUuid.SERVICE_DEVICE_INFORMATION, BleUuid.CHAR_NODE,
						mBluetoothAdapter.getAddress());
				Log.i(TAG, "local mac" + mBluetoothAdapter.getAddress());
			}
		}
	...
	private IBLECentralService.Stub mStub = new IBLECentralService.Stub() {
		...
		@Override
		public void addListener(IBLECentralCallback callback, IWearableListener listener) throws RemoteException {
			String node = mBluetoothAdapter.getAddress();
		...
		@Override
		public void removeListener(IBLECentralCallback callback, IWearableListener listener) throws RemoteException {
			String node = mBluetoothAdapter.getAddress();
	...
	private void onPostMessageReceived(MessageEventHolder messageEventHolder) {
		...
		for (WearableListenerItem listener : mWearableListenerList) {
			...
			if (listener.getNode().equals(messageEventHolder.getSourceNodeId())
					&& messageEventHolder.getPackageName().equals(listener.getPackageName())) {
				try {
	...
	private void onPeerDisconnected(NodeHolder nodeHolder) {
		...
		for (NodeListenerItem listener : mNodeListenerList) {
			Log.i(TAG,
					"手机端：通知上层onPeerDisconnected, node =  " + nodeHolder.getId() + ", displayName = "
							+ nodeHolder.getDisplayName());
			try {
