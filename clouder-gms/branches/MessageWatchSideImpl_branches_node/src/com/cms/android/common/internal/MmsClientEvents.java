...
public class MmsClientEvents {
	...
	public void connectFailed(ConnectionResult connectionResult) {
		...
		synchronized (this.mListeners) {
			...
			Iterator<OnConnectionFailedListener> localIterator = this.mListeners.iterator();
			while (localIterator.hasNext()) {
				MobvoiApiClient.OnConnectionFailedListener localOnConnectionFailedListener = (MobvoiApiClient.OnConnectionFailedListener) localIterator
						.next();
				localOnConnectionFailedListener.onConnectionFailed(connectionResult);
			}
		}
	}
	...
	public void registerConnectionFailedListener(
			MobvoiApiClient.OnConnectionFailedListener paramOnConnectionFailedListener) {
		Assert.neNull(paramOnConnectionFailedListener);
		Log.i(TAG, "register connection failed listener");
		synchronized (this.mListeners) {
			if (this.mListeners.contains(paramOnConnectionFailedListener)) {
				Log.w("MmsClientEvents", "duplicated register connection failed listener : "
						+ paramOnConnectionFailedListener + ".");
				return;
			}
			this.mListeners.add(paramOnConnectionFailedListener);
		}
	}
