...
public class MmsHandleCallback implements Handler.Callback {
	...
	private Intent createExplicitFromImplicitIntent(Context context,
		...
		if (resolveInfo == null || resolveInfo.size() != 1) {
			Log.e(TAG, "found service size = "
					+ (resolveInfo == null ? 0 : resolveInfo.size()));
			return null;
		}
	...
	public boolean bindService(String action,
			MmsClient.MmsServiceConnection serviceConnection) {
		if (TextUtils.isEmpty(action) || serviceConnection == null) {
			return false;
		}
		try {
			Intent serviceIntent = createExplicitFromImplicitIntent(
					this.mContext, new Intent(action));
			if (serviceIntent != null) {
				this.mContext.bindService(serviceIntent, serviceConnection,
						Context.BIND_AUTO_CREATE);
				return true;
			} else {
				Log.e(TAG, "手表端CMS服务不存在");
				return false;
			}
		} catch (Exception e) {
	...
	public void unbindService(String action,
			MmsClient.MmsServiceConnection serviceConnection) {
		if (TextUtils.isEmpty(action) || serviceConnection == null) {
			return;
		}
	...
	static final class Proxy {
		...
		static ComponentName setComponentName(Proxy proxy,
		...
		public boolean hasConn(
				MmsClient.MmsServiceConnection mmsServiceConnection) {
