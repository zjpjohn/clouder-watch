...
public class RFCOMMServerService extends Service {
	...
	private Handler mTimeHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case MESSAGE_TYPE_TIMER_OCCUR:
				if (isRFCOMMExpired()) {
					LogTool.e(TAG, "手机端：当前已经超时,则关闭RFCOMM通道");
					stopThread(RFCOMM_DISCONNECT_CAUSE_EXPIRED);
				} else {
					LogTool.d(TAG, "手机端：当前未超时,则不做任何处理");
				}
				break;
	...
	private class AcceptThread extends Thread {
		...
		public AcceptThread() {
			...
			try {
				...
				LogTool.d(TAG, "当前监听地址为 " + mBluetoothMacAddress);
				setState(STATE_LISTEN);
			} catch (IOException e) {
				setState(STATE_NONE);
				LogTool.e(TAG, "AcceptThread 监听RFCOMM接口通道创建异常", e);
				return;
			} // 服务仅监听
			serverSocket = tmp;
			LogTool.d(TAG, "手机端：serverSocket hashcode = " + serverSocket.hashCode());
		}
			...
			while (mState != STATE_CONNECTED) {
				try {
					...
					if (mRfcommServerCallback != null) {
						LogTool.d(TAG, "RFCOMM Thread Socket 已经准备就绪");
						boolean isSuccess = mRfcommServerCallback.onRFCOMMSocketReady(mBluetoothMacAddress);
						// count++;
						// if (!isSuccess) {
						// LogTool.e(TAG, "手机端告知手表端准备就绪失败,则重新告知 count = ");
						// if (count == 3) {
						// LogTool.e(TAG, "已告知3次都失败,则断开");
						// break;
						// } else {
						// continue;
						// }
					...
					setState(STATE_LISTEN);
					LogTool.d(TAG, "RfcommThread serverSocket accept waiting...");
					socket = serverSocket.accept();
				} catch (IOException e) {
					LogTool.d(TAG, "Accept Thread IOException", e);
					setState(STATE_NONE);
				} catch (RemoteException e) {
					LogTool.d(TAG, "Accept Thread RemoteException", e);
					setState(STATE_NONE);
				} catch (NullPointerException e) {
					LogTool.d(TAG, "Accept Thread NullPointerException", e);
					setState(STATE_NONE);
				}
				// If a connection was accepted
				LogTool.d(TAG, "RFCOMM Thread serverSocket pass accept and mState = " + mState);
				if (socket != null) {
					synchronized (RFCOMMServerService.this) {
						switch (mState) {
						case STATE_LISTEN:
						case STATE_CONNECTING:
							LogTool.e(TAG, "STATE_LISTEN or STATE_CONNECTING connectedRfcomm");
						...
						case STATE_CONNECTED:
							...
							// new socket.
							LogTool.e(TAG, "STATE_NONE or STATE_CONNECTED socket close");
							try {
								socket.close();
							} catch (IOException e) {
								LogTool.e(TAG, "Could not close unwanted socket", e);
							}
							break;
						}
					}
	...
	private class ConnectedThread extends Thread {
		...
		public ConnectedThread(BluetoothSocket socket) {
			...
			try {
				...
				LogTool.e(TAG, "ConnectedThread InputStream and OutputStream Exception", e);
			}
			inStream = tmpIn;
			outStream = tmpOut;
		}
		...
		public void run() {
			...
			while (!isStop) {
				...
				try {
							...
							if (TransportParser.checkDataPack(ABCDEFG)) {
								...
								if (mRfcommServerCallback != null) {
									...
									case TransportParser.REQUEST_PARSER_TYPE:
										...
										mRfcommServerCallback.onDataReceived(ABCDEFG);
										break;
									case TransportParser.RESPONSE_PARSER_TYPE:
										LogTool.d(TAG, "type-> RESPONSE");
										mRfcommServerCallback.onDataSent(ABCDEFG);
										break;
					...
					LogTool.e(TAG, "PackageData Exception: " + message, e);
					state = LinkDecoderState.ReadA;
					// 即不是主动断开连接的,则发生了异常
					if (future != null) {
						future.cancel(false);
					...
					if (!isStop) {
						LogTool.d(TAG, "isStop = " + isStop + ",非主动关闭,则重新启动RFCOMM Server服务,通知手表端手机端发生异常");
						reconnect();
		...
		public boolean writeBytes(byte[] bytes) {
			try {
				...
				LogTool.e(TAG, "手机端：写入Byte数组异常", e);
			}
			return false;
		}
	...
	private synchronized void startThread() {
		LogTool.i(TAG, "创建线程,准备启动RFCOMM服务");
	...
	private boolean writeBytes(byte[] out) {
		LogTool.d(TAG, "RFCOMM 写入 bytes = " + (out == null ? 0 : out.length));
	...
	private void validateExpireTime() {
		...
		LogTool.d(TAG, "手机端：过期时间更新为" + sdf.format(mExpiredTime));
	}
	...
	private boolean isRFCOMMExpired() {
		LogTool.d(TAG, "判定是否超时 当前时间 = " + sdf.format(new Date()) + " 超时时间 = " + sdf.format(mExpiredTime));
